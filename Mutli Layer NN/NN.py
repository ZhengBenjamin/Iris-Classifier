...
class Model(nn.Module):
  ...
  def __init__(self, x, y):
    ...
    # print(f"X Train: {x_train} Y Train: {y_train} X Test: {x_test} Y Test: {y_test}")
    ...
    self.input = nn.Linear(len(x[0]), 64) # Input 
    self.layer1 = nn.Linear(64, 32) # Hidden
    self.output = nn.Linear(32, 3) # Output
    self.relu = nn.ReLU()
  ...
  def train(self, epochs):
    ...
    optimizer = torch.optim.Adam(self.parameters(), lr=0.0001) # Adam optimizer
